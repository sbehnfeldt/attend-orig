...
let Schedules = (function () {
    ...
    function update(s) {
        for (let i = 0; i < records[s.StudentId].length; i++) {
    ...
    let b0 = new $.fn.dataTable.Buttons(table, {
        buttons: [{
            ...
            "action": function (e, dt) {
                ...
                let msg      = (1 === selected.length) ? 'Are you sure you want to delete this student record?' : 'Are you sure you want to delete these ' + selected.length + ' student records?';
                if (confirm(msg)) {
...
let StudentPropsDlg = (function (selector) {
    ...
    Classrooms.subscribe('classrooms-loaded', function (classrooms) {
        for (let p in classrooms) {
            let $option = $('<option>').val(p).text(classrooms[p].Label);
