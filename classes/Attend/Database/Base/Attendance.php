...
abstract class Attendance implements ActiveRecordInterface
{
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'student_id':
                        ...
                        break;
                    case 'check_in':
                        $stmt->bindValue($identifier, $this->check_in, PDO::PARAM_INT);
                        break;
                    case 'check_out':
                        $stmt->bindValue($identifier, $this->check_out, PDO::PARAM_INT);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(AttendanceTableMap::COL_ID)) {
            $criteria->add(AttendanceTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(AttendanceTableMap::COL_STUDENT_ID)) {
            $criteria->add(AttendanceTableMap::COL_STUDENT_ID, $this->student_id);
        }
        if ($this->isColumnModified(AttendanceTableMap::COL_CHECK_IN)) {
            $criteria->add(AttendanceTableMap::COL_CHECK_IN, $this->check_in);
        }
        if ($this->isColumnModified(AttendanceTableMap::COL_CHECK_OUT)) {
            $criteria->add(AttendanceTableMap::COL_CHECK_OUT, $this->check_out);
        }
